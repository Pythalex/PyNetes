...
class Application(Tk):
	...
	def initApp(self):
				# Self config
		self.tk_setPalette(background='#2d2d30', foreground='#ececec', activeBackground='#212123', activeForeground="#ececec")
		...
		self.canvas 						= Canvas(self.frame1, width = self.WIDTH, height = self.HEIGHT, bg = "#000000")  # black background
		self.canvas.origine 				= (self.WIDTH / 2, self.HEIGHT / 2)
		...
		self.predef_planets_frame 			= LabelFrame(self.frame2, text = "Planètes pré-enregistrées")
		self.planetes_import 				= lire_save ("Planetes/planetes.txt")
		var_create = globals()
		compt = 0
		for elems in self.planetes_import:
			var_create[elems.nom + str(compt) + "v"] = IntVar()
			var_create[elems.nom + str(compt)] = Checkbutton(self.predef_planets_frame, text = elems.nom, variable = var_create[elems.nom + str(compt) + "v"])
			var_create[elems.nom + str(compt)].grid(row = compt, column = 0)
			compt += 1
		self.predef_planets_button = Button(self.predef_planets_frame, text = "Appliquer", command = self.ajouter_planete_terre)
		...
		self.predef_planets_button.grid(row = compt, column = 0)
	...
	def ajouter_planete_terre(self):
		var_create = globals()
		for i in range(len(self.planetes_import)):
			if(var_create[self.planetes_import[i].nom + str(i) + "v"].get() == 1):
				self.ajouter_planete 		(self.planetes_import[i])
	...
	def ajouter_planete(self, predef = False):
